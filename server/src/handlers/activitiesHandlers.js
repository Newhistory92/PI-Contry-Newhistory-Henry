...
const getActivityHandler = async (req, res) => {
  try {
    ...
    res.status(400).json({ error: error.message });
     // Envía la respuesta de error con el código de estado 400 y el mensaje de error en formato JSON
  }
};
...
const postActivityHandler = async (req, res) => {
  ...
  try {
    if (countries.length === 0) throw new Error('Debe estar asociado a un país');
     // Verifica si la actividad está asociada a algún país, los paises se alamcenan en un array,
